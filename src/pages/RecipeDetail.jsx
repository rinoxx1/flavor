...
const RecipeDetail = () => {
  ...
  const getRecipe = async (id) => {
    try {
      ...
      setRecipe(data?.recipe); // Update this line to access the recipe property

      if (data?.recipe?.label) {
        const recommend = await fetchRecipes({ query: data.recipe.label, limit: 5 });
        ...
        <div className='flex gap-10 items-center justify-center px-4'>
          <div className='flex flex-col justify-between'>
            <span className='text-white text-center border border-gray-500 py-1.5 px-2 rounded-full mb-2'>
              {recipe?.calories ? recipe.calories.toFixed(2) : '0'} 
            </span>
          ...
          <div className='w-full md:w-2/4 md:border-r border-slate-800 pr-1'>
            ...
            <div className='flex flex-col gap-3 mt-20'>
              ...
              <div className='flex flex-wrap gap-4'>
                {recipe?.healthLabels && recipe.healthLabels.map((item, index) => (
